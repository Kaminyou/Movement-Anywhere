...
def draw_one_variable(predict_variable, gt_variable, title, groups, scaling, unit='ms'):
    ...
    df = pd.DataFrame(
        {
            'gt_variable': gt_variable,
            'predict_variable': predict_variable,
            'groups': groups,
        },
    )
    ...
    plt.title(f'{title} \n L1={l1_err:.3f} {unit} || MSE={mse_err:.3f} {unit}² || corr={corr_coef:.3f} || err={err_mean * 100:.3f}%')  # noqa
    plt.grid()
    plt.legend()
    plt.show()
    # return df
