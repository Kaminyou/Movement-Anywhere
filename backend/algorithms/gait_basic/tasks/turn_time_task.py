...
class TurnTimeTaskRunner(Runner):
    def __init__(
        ...
        # input
        self.output_output_3dkeypoint_path_remote = os.path.join(SYNC_FILE_SERVER_RESULT_PATH, self.submit_uuid, 'out', '3d', f'{self.file_id}.mp4.npy')
        self.output_output_3dkeypoint_path_local = os.path.join(WORKER_WORKING_DIR_PATH, self.submit_uuid, 'out', '3d', f'{self.file_id}.mp4.npy')
    ...
    def fetch_data(self):
        ...
        self.data_synchronizer.download(
            src=self.output_output_3dkeypoint_path_remote,
            des=self.output_output_3dkeypoint_path_local,
        )
    ...
    def execute(self):
        tt, raw_tt_prediction = turn_time_simple_inference(
            ...
            path_to_npz=self.output_output_3dkeypoint_path_local,
